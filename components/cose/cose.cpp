...
esp_err_t CoseCrypto::sign(const vector<uint8_t>& payload, 
    ...
    // 2. Build Sig_structure and Hash it (SHA-256)
...
esp_err_t CoseCrypto::verify(const vector<uint8_t>& cose_data, 
    ...
    // 6. Reconstruct Sig_structure and Hash
    ...
    // 7. Verify Signature using MbedTLS
    mbedtls_ecp_group grp;
    mbedtls_ecp_point Q;
    mbedtls_mpi r, s;
